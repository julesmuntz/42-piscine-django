...
def search(request, page_title):
	if (
		not Movies.objects.exists()
		or not Planets.objects.exists()
		or not People.objects.exists()
	):
		return render(
			request,
			"d05/templates/search.html",
			{"title": page_title, "empty_tables": True},
		)

	genders = list(
		People.objects.values_list("gender", flat=True)
		.distinct()
		.exclude(gender__isnull=True)
	)
	sorted_genders = sorted(
		genders, key=lambda x: ({"male": 0, "female": 1}.get(x, 2), x)
	)
	...
	if request.method == "POST":
		...
		if all([min_date, max_date, min_diameter, gender]):
			...
			for character in characters:
				movies = character.movies.filter(
		...
		context.update(
			{"has_searched": True, "results": results, "no_results": len(results) == 0}
		)
